...
export default class player {
    ...
    private start = (song: Song, from_playlist?: boolean, id?: number) => {
        ...
        this.ffplay = spawn(process.env.FFPLAY_PATH || "ffplay", ["-nodisp", "-autoexit", song.start != 0 ? "-ss "+song.start : "", song.end != 0 && (song.end-song.start)>0 ? "-t "+(song.end-song.start) : "", join(cfg.song_folder, song.filename)], {stdio: 'ignore'});
        this.song = song;
        if (from_playlist) {
